...
export default function AnimeCard({ anime }: { anime: any }) {
  ...
  useEffect(() => {
    // 1. Lấy user và kiểm tra trạng thái yêu thích ngay khi load card
    const checkStatus = async () => {
      ...
      if (user) {
        const { data } = await supabase
          .from('favorites') // Tên bảng bạn vừa tạo
          .select('id')
          .eq('user_id', user.id)
  ...
  // Logic Hover lấy content (movie.content)
  ...
  return (
    <div className="relative flex flex-col block cursor-pointer font-roboto group" onMouseEnter={handleMouseEnter} onMouseLeave={() => { hoverTimeoutRef.current = setTimeout(() => setIsHovered(false), 150); }}>
      <Link to={`/phim/${anime.slug}`}>
        <div className="relative aspect-[2/3] overflow-hidden bg-[#1a1a1a] rounded-sm">
          ...
          <button onClick={toggleFavorite} className={`absolute top-2 right-2 z-30 w-7 h-7 flex items-center justify-center rounded-full bg-black/60 border border-white/10 transition-opacity ${isHovered ? 'opacity-100' : 'opacity-0 md:opacity-0'}`}>
            <span className={isFavorite ? 'text-[#d9534f]' : 'text-white'}>{isFavorite ? '♥' : '♡'}</span>
          </button>
        </div>
        <div className="pt-2 pb-1">
          <h2 className="text-[14px] font-bold text-[#e5e5e5] line-clamp-2 leading-[1.3] mt-1 group-hover:text-[#d9534f] transition-colors">{anime.title}</h2>
        </div>
      </Link>
      ...
      {isHovered && (
        <div className="absolute top-0 z-50 hidden w-[280px] pl-2 md:block left-full animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-[#111622] p-4 border shadow-2xl border-white/10 rounded-sm">
            ...
            <button onClick={toggleFavorite} className={`w-full py-2 font-bold text-center text-[12px] uppercase transition-colors rounded-sm border ${isFavorite ? 'bg-[#d9534f] border-[#d9534f] text-white' : 'bg-transparent border-white/20 text-white hover:bg-[#d9534f]'}`}>
