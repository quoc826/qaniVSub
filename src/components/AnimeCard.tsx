...
import { getSessionOnce } from '../services/sessionCache';
...
export default function AnimeCard({ anime, priority = false }: { anime: any, priority?: boolean }) {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        // Dùng session cache chung — chỉ gọi Supabase 1 lần cho toàn bộ cards
        const currentUser = await getSessionOnce();
        if (cancelled) return;
        setUser(currentUser);
        ...
        console.warn('Lỗi tải trạng thái yêu thích:', error);
      }
    };
  ...
  const handleMouseEnter = async () => {
    ...
    if (!movieDetail && !isLoadingContent) {
      ...
      // phimApi.getAnimeDetail đã được cache — lần 2 trả về ngay lập tức
  ...
  const cleanContent = movieDetail?.content
    ? movieDetail.content.replace(/<[^>]*>?/gm, '')
    : 'Đang tải...';
  ...
  return (
    ...
    >
      ...
      {isHovered && (
        <div className="absolute top-0 z-50 hidden w-[280px] pl-2 md:block left-full animate-in fade-in zoom-in-95 duration-150">
