  ...
  return (
    <header className="sticky top-0 z-50 bg-black border-b shadow-xl border-white/10" ref={headerRef}>
      <div className="flex items-center justify-between h-20 px-4 mx-auto max-w-7xl font-roboto">
        <Link to="/"><img src="/shigy.gif" alt="Logo" className="transition-transform h-14 hover:scale-105" /></Link>

        <nav className="items-center hidden space-x-2 text-sm font-bold uppercase md:flex font-oswald">
        ...
        <div className="flex items-center gap-4">
          <form onSubmit={handleSearch} className="relative hidden sm:block">
            <input type="text" value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="Tìm Anime..." className="bg-[#1a1a1a] border border-white/10 text-white text-sm px-4 py-2 w-64 focus:border-[#d9534f] outline-none rounded-sm" />
          </form>
          ...
          {user ? (
            <div className="flex items-center gap-3">
              <div className="flex flex-col items-end">
                <span className="text-white text-[12px] font-bold uppercase font-oswald leading-none">
                  Hi, {user.user_metadata?.display_name || "Thành viên"}
                </span>
                <button 
                  onClick={handleLogout}
                  className="text-[#d9534f] text-[10px] uppercase font-bold hover:text-white transition-colors"
                >
                  Đăng xuất
                </button>
              </div>
              <div className="w-8 h-8 rounded-full bg-[#d9534f] flex items-center justify-center text-white font-bold text-xs border border-white/10">
            ...
            <Link 
              to="/dang-nhap" 
              className="bg-[#d9534f] text-white px-5 py-2.5 font-oswald font-bold uppercase text-xs hover:bg-white hover:text-black transition-all shadow-[0_0_10px_rgba(217,83,79,0.3)] rounded-sm"
            >
              Đăng Nhập
            </Link>
          )}
